...
popupManager.addEventListener(`click`, function (event) {
    const [addCardLayer, editUserLayer, imageLayer, changeAvatarLayer] =
        popupManager.querySelector(`.popup`).children;
    ...
    } else if (event.target.classList.contains(`user-info__photo`)) popup.open(changeAvatarLayer);
});